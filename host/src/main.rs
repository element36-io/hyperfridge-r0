...
#[test]
fn do_profiling() {
    //let profile_output_path = PathBuf::from("./profile-output");

    println!(".... profiling start");
    let mut profiler = risc0_zkvm::Profiler::new("./profile-output", HYPERFRIDGE_ELF).unwrap();

    let env = risc0_zkvm::ExecutorEnv::builder()
        .write(&crate::SIGNED_INFO_XML_C14N).unwrap()
        .write(&crate::AUTHENTICATED_XML_C14N).unwrap()
        .write(&crate::SIGNATURE_VALUE_XML).unwrap()
        .write(&crate::ORDER_DATA_XML).unwrap()
        .write(&crate::BANK_X002_MOD).unwrap()
        .write(&crate::BANK_X002_EXP).unwrap()
        .write(&crate::USER_PRIVATE_KEY_E002_PEM).unwrap()
        .trace_callback(profiler.make_trace_callback())
        .build().unwrap();

    let exec = risc0_zkvm::default_executor();
    exec.execute_elf(env, HYPERFRIDGE_ELF).expect("execution failed");

    profiler.finalize();
    let report = profiler.encode_to_vec();
    println!(".... write profile size {}",report.len());
...
#[cfg(test)]
mod tests {
    use crate::{proove_camt53,HYPERFRIDGE_ELF};
    #[test]
    fn do_main() {
